...
\end{description}
% section triggertypen (end)
\section{DML Trigger auf Tabellen} % (fold)
...
\section{Trigger Beispiele} % (fold)
...
\end{lstlisting}
% section trigger_beispiele (end)
% section dml_trigger_auf_tabellen (end)
% chapter trigger (end)
...
Fragmente widersprechen, resultiert eine leere Relation. Vor einer möglichen Reduktion wird das Distributivgesetz 
angewendet : \\
...
\chapter{Replikation II ( Mat Views)} % (fold)
...
\end{description}
% section komponenten (end)
\section{Benutzer und Rollen} % (fold)
...
\subitem Refresher
% section benutzer_und_rollen (end)
\section{Master Site Strukturen} % (fold)
...
\end{verbatim}
% section master_site_strukturen (end)
\section{Materilized View Site Strukturen} % (fold)
...
auf.
% section materilized_view_site_strukturen (end)
\section{Konfliktauflösung} % (fold)
...
% section konfliktaufl_sung (end)
% chapter replikation_ii (end)
...
\section{Idee des Semantic Webs} % (fold)
...
\section{Ziele des Semantic Webs} % (fold)
...
ermöglichen.
\end{itemize}
% section ziele_des_semantic_webs (end)
% chapter semantic_web (end)
\section{Datenmodell WWW und semantic web} % (fold)
\label{sec:datenmodell_www}
\pic{dmweb.png}


% section datenmodell_www (end)

\section{Technologieen} % (fold)
\label{sec:technologieen}

\subsection{RDF - Resource Description Framework} % (fold)
...
\item rdf:object
\end{itemize}
% subsection rdf_resource_description_framework (end)
\section{RDF und OWL} % (fold)
...
\end{itemize}
% section rdf_und_owl (end)
% section technologieen (end)
...
\end{tabular}
\chapter{Lab 3 - Verteilter Datenbankentwurf}
...
\section{Setup Master Site} % (fold)
...
\end{lstlisting}
% section setup_master_site (end)

\section{Setup Materialized View Site} % (fold)
...
\section{Erstellen von Master Group} % (fold)
...
\end{lstlisting}
% section erstellen_von_master_group (end)

\section{Erstellen von Master View Group} % (fold)
...
CREATE MATERIALIZED VIEW mvdbsx2.filialen
REFRESH FAST WITH PRIMARY KEY FOR UPDATE
AS SELECT * FROM mvdbsx2.filialen@telesto.janus.fhnw.ch;
...
\section{Lösungen der Konflikte} % (fold)
\label{sec:l_sungen_der_konflikte}

\subsection{Update Konflikt Lösung}
Im folgenden soll eine Update-Konflikt-Lösung\footnote{\url{http://docs.oracle.com/cd/E11882_01/server.112/e10707/rarconflictres.htm#REPMA354}} definiert werden, damit bei einem Update, ein allfälliger Konflikt automatisch behoben wird.
...
% section l_sungen_der_konflikte (end)

% section erstellen_von_master_view_group (end)
...
GROUP BY mnr;
\end{lstlisting}
\lstinputlisting[language=java,style=java,firstline=18,style=java]{Labs/Lab11_MapReduce/GroupByWithCount.java}
\section{Join}
\begin{lstlisting}[style=sql]
SELECT *
FROM registrierungen JOIN mitglieder USING (mnr);
\end{lstlisting}
\lstinputlisting[language=java,style=java,firstline=19,style=java]{Labs/Lab11_MapReduce/Join.java}
\lstinputlisting[language=java,style=java,firstline=8,style=java]{Labs/Lab11_MapReduce/DatasetTagger.java}
\subsection{Lösungsbeschrieb}
